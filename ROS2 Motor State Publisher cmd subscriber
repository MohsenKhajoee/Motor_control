...
#define PULSES_PER_REV 972  // Pulses per revolution based on encoder spec.
...
Encoder(int Pin_A, int Pin_B) : pin_A(Pin_A), pin_B(Pin_B), position(0), revolusion(0), lastTime(0), angularSpeed(0), currentAngle(0), lastAngle(0) {}
    ...
    void init() {
        ...
        encoder.attachFullQuad(pin_A, pin_B); // Initialize ESP32Encoder
        encoder.clearCount(); // Reset count to 0
    }



    float getRadian() {
        return (encoder.getCount() / (float)PULSES_PER_REV) * 2 * PI;
    }
